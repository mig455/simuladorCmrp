...
import { RegistroCmrpExamenDetalleDTO } from 'src/app/Models/ExamenDetalleDTO';
import { RegistroCmrpExamenRespuestaDTO } from 'src/app/Models/ExamenDTO';
...
export class ExamenPreguntaComponent implements OnInit {
  ...
  public RegistroEnvioRespuesta:RegistroCmrpExamenRespuestaDTO={
    id:0,
  ...
  public DetalleRespuestaEnvio:RegistroCmrpExamenDetalleDTO={
    id:0,
    idSimuladorCmrpExamen:0,
    idSimuladorCmrpDominio:0,
    idSimuladorCmrpTarea:0,
    idSimuladorCmrpPregunta:0,
    ejecutado:false,
    idSimuladorCmrpPreguntaRespuesta:0,
    puntaje:0,
    idAspNetUsers:'',
    usuario:''
  }
    ...
    this._ExamenService.ObtenerExamenDetallePreguntaPorId(this.IdExamen).subscribe({
      next:(x)=>{
        ...
        if(this.ListaPreguntas.length==0){
          ...
          //El modo examen dura 150 minutos
          this.TiempoSegundoReversa=9000-x.tiempo;
...
EnviarRespuesta(i:number){
  ...
  this.RegistroEnvioRespuesta.idSimuladorCmrpModo=3,
  ...
  this.ListaPreguntas[i].pregunta.respuesta.forEach((x:any)=>{
    if(x.respuestaSelecionada==1){
      this.DetalleRespuestaEnvio.idSimuladorCmrpPreguntaRespuesta=x.id;
      this.DetalleRespuestaEnvio.id=this.ListaPreguntas[i].id;
      this.DetalleRespuestaEnvio.idSimuladorCmrpExamen=0;
      this.DetalleRespuestaEnvio.idSimuladorCmrpDominio=0;
      this.DetalleRespuestaEnvio.idSimuladorCmrpTarea=0;
      this.DetalleRespuestaEnvio.idSimuladorCmrpPregunta=this.ListaPreguntas[i].idSimuladorCmrpPregunta;
